...
|---|---|
| 범용 검색 | 검색엔진 색인을 본다 |
| 직접 조회 | 발행처 사이트 검색을 직접 호출한다 |

**직접 조회는 일반 웹 검색으로 대체가 안 된다.** 발행처 검색 결과 페이지는
`noindex` 라 검색엔진이 원리적으로 색인하지 않는다. 직접 열어야만 보인다.

그런데 평문 요청은 대부분 막힌다. **스크래핑 도구가 붙어 있으면 좋다.**
Firecrawl MCP 같은 것이다.

| 있을 때 | 없을 때 |
|---|---|
| 발행처 다섯 곳을 다 본다 | 대부분 조회 실패로 남는다 |
| 독립 출처를 잡을 수 있다 | 검증 분류 근거가 얕아진다 |

없어도 돈다. 못 본 것은 **조회 실패**로 적히고 해당 없음으로 적히지 않는다.
나중에 도구를 붙여 다시 보면 된다.

무료 대체재를 찾아봤는데 쓸 만한 게 없었다.
리더 프록시, 브라우저 도구, 내장 fetch, curl 전부 막혔다.
검색 API를 바꾸는 것으로는 해결되지 않는다. `noindex` 는 색인에 없으니 그렇다.

**막힌 것을 성공으로 착각하기 쉽다.** 막혀도 200을 주는 경우가 있다.
Cloudflare 화면이나 사이트 첫 화면이 200으로 온다.
2단계의 조회 성공 판정이 이걸 걸러낸다. 상태 코드로 판정하지 않는다.
